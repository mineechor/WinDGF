...
#     func_addr = function.start_ea
    ...
    for block in cfg:
        bbweight[block.start_ea] = [block.end_ea,-1]
    ...
    for node in nx_graph.nodes():
        for key in tempdict:
            if nx.has_path(nx_graph,node,key):
                dis = nx.dijkstra_path_length(G=nx_graph, source=node, target=key)+5*tempdict[key]
...
# print(hex(min_ea))
# print(bbweight)
f = open("D:\\final\\2022\\angr\\angr\\cms\\test\\rle\\xnview3weight.txt","w")
for key in bbweight:
    dist = bbweight[key][1]
    if dist>maxweight:
        maxweight = dist
