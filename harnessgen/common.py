...
class TraceElement:
    ...
    def update_calltrace(self, dst_sym, args,args_type, args_dump, pointer):
    ...
    def update_rettrace(self, ret_val,ret_type):
...
class FunctypeManager:
    ...
    def get(path) -> Functype:
        ...
        # rely = 'D:\\2022\\final\\w2\\winnie\\harnessgen\\util\\2.txt'
        # print(command)
...
class Trace:
    ...
    def parse_call(self, chunk: bytes, tid, cid, parse_args=True) -> TraceElement:
        ...
        if fi:
            dst_sym = fi.name
            numargs = fi.argsize
            # args_type = [arg['type'] for arg in fi['args']]
            args_type = fi.args
        ...
        if parse_args:
            ...
            for x in range(numargs):
                ...
                if current_arg[0][1] == "DP":
                    ...
                    # 可以利用same增加判断参数在函数进入退出前后有无变化，方便后续判断data_dependency
                else:
                    args_dump.append(None)
                    pointer.append(None)

        te = TraceElement("call", src_addr, dst_addr, src_module, dst_module)
        te.update_calltrace(dst_sym, args,args_type,args_dump, pointer)
        # print(te)
        return te
    ...
    def parse_ret(self, chunk: bytes) -> TraceElement:
        ...
        src_addr = int(src[0], 16)
        dst_addr = int(dst[0], 16)
        fi = self.find_function(src_addr)
        # print(fi)
        # print(src_addr)
        if fi:
        ...
        te.update_rettrace(ret_val,ret_type)
...
class Synthesizer:
    def __init__(self, trace_pn: str, dump_pn: str, functype_pn: str, start_func=None, sample_name: str = None):
        ...
        self.functype_manager = FunctypeManager()
        # print(self.functype_manager)
        self.trace = Trace(self.trace_pn, self.dump_pn, self.trace_tid, self.start_cid)
        self.defined_types, self.defined_funcs,self.defined_loads = self.typedef()
    ...
    def emit_code(self, out_pn=None):
        ...
        fuzzme = fuzzme.replace("{loaddef}", '\n'.join(self.defined_loads))
    ...
    def typedef(self):
        ...
        return defined_types, defined_funcs, defined_loads
    ...
    def search_pointer(self, addr,type,passed_cid=10000, internal_use=False):
        """
        ...
        for cid in self.trace.cid_sequence:
            ...
            retval = rettrace.ret_val
            rettype = rettrace.ret_type
            args = calltrace.args
            args_type = calltrace.args_type
            args_dump = calltrace.args_dump
            args_ptr = calltrace.pointer
            if retval == addr and addr != 0 and rettype == type:
                if not internal_use:
                        print("[*] Passed argument at [cid:%d] [arg:%dth]" % (cid, x))
                else:
                        result.append(("ret", cid, retval))
            else:
                for x in range(len(args)):
                    current_arg = args[x][0][0]
                    if current_arg == addr and type == args_type[x]:
                        if not internal_use:
                            print("[*] Passed argument at [cid:%d] [arg:%dth]" % (cid, x))
                        else:
            ...
            for x in range(len(args)):
                if args[x][0][1] == 'DP' and "*" in args_type[x]:
        ...
        if not internal_use:
            ...
            return result
    def search_data(self, addr,type,passed_cid=10000, internal_use=False):
        result = []
        for cid in self.trace.cid_sequence:
            if cid > passed_cid:
                continue
            ...
            args_type = calltrace.args_type
            rettype = rettrace.ret_type
            if retval == addr and rettype==type:
                if not internal_use:
                        print("[*] Passed argument at [cid:%d] [arg:%dth]" % (cid, x))
                else:
                        result.append(("ret", cid, retval))
    ...
    def ret_arg_code(self, cid, args, args_dump, args_type, args_ptr):
        ...
        for x in range(len(args)):
            ...
            if args[x][0][1] == 'DP':
                ...
                # print(strings(args_dump[x][0]))
                # print(args_dump[x][0])
                # print(x)
                # print(first_string)
                # print(x)
                # print(self.sample_name.encode())
                if self.sample_name:
                    if self.sample_name.encode() in first_string:
                        ...
                        dumped = hex(u32(args_dump[x][0]))
                _type = args_type[x]
                type = args_type[x].replace("*", "")
                #print(_type)
                # print(dumped)

                # 1-1) infer chuck of actual sample is used in the function
                # TODO
                # print(hex(u32(args_dump[x][0])))
                # print(hex(u32(args_dump[x][1])))
                # 2) we allocate heap if pointed value is 0
                # print(dumped)
                # if dumped == '0x0' and '*' in _type:
                ...
                #    If there is, we reuse the pointer
                    ...
                    # print(self.defined_pointer)
                    if args[x][0][0] not in self.defined_pointer:
                        # print(hex(args[x][0][0]))
                        result = self.search_pointer(args[x][0][0],_type,cid, internal_use=True)
                        # print(result)
                        # print(cid)
                        result_arg = self.check_searched_result(result, "arg")
                        result_ret = self.check_searched_result(result,"ret")
                        ...
                        elif result is not None and result_ret is not None and result_ret[1]!=cid:
                            ret_cid = result_ret[1]
                            print(ret_cid)
                            print(cid)
                            self.defined_pointer[args[x][0]
                                                 [0]] = "c%d_a%d" % (cid, x)
                            need_to_define.append(
                                "%s c%d_a%d = c%d_ret;" % (_type, cid, x, ret_cid))
                        else:
                            self.defined_pointer[args[x][0]
                                                 [0]] = "c%d_a%d" % (cid, x)
                            if "*" in _type:
                                ...
                                need_to_define.append(
                                    "%s c%d_a%d = %s;" % (_type, cid, x, hex(args[x][0][0])))
            ...
            # raw data
                ...
                # TODO: consider data-type when unpack
                _type = args_type[x]
                if args[x][0][0] not in self.defined_data:
                    result = self.search_data(args[x][0][0],_type,cid, internal_use=True)
                    result_arg = self.check_searched_result(result, "arg")
                    result_ret = self.check_searched_result(result,"ret")
                    if result is not None and result_ret is not None and result_ret[1]!=cid:
                            ret_cid = result_ret[1]
                            self.defined_data[args[x][0]
                                                 [0]] = "c%d_a%d" % (cid, x)
                            need_to_define.append(
                                "%s c%d_a%d = c%d_ret;" % (_type, cid, x, ret_cid))
                    else:
                    ...
                    need_to_define.append("")
                arguments.append(self.defined_data[args[x][0][0]])
                # arguments.append(hex(raw_value))
        # print(need_to_define)
        # print(arguments)
        return need_to_define, arguments

    def ret_typedef_func(self, funcname, args_str, convention, ret_type):
        # e.g., typedef int (__stdcall *avformat_get_riff_audio_tags_func_t)();
        return "typedef %s (%s *%s)(%s);" % (ret_type, convention, funcname.decode(), args_str)

    def ret_defined_func(self, funcname):
        return (b"    %s %snew;" % (funcname, funcname)).decode()
    def ret_defined_load(self, funcname):
        if b"sub_" in funcname:
            off = funcname.decode()[4:]
            return (b"    %snew = (%s)(hinstLib+0x%s);"%(funcname,funcname,off.encode())).decode()
        return (b"    %snew = (%s)GetProcAddress(hinstLib,\"%s\");"%(funcname,funcname,funcname)).decode()
