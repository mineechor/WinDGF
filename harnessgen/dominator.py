...
class Dominator(object):
    ...
    def dominator(self):
        ...
        # 所有cid >= start_cid&&cid<=end_cid的数量
        print(" >> Total unique harness functions: %d" % (len(self.trace.all_callers)))
        # func_counter中最高的计数数值（在这里最高的就代表是LCA或其祖先）
        ...
        """
        # 只被调用一次的候选harness被认为是good，其他是bad
        print("\n[*] Dominator analysis")
        candidate = {}
        candidate["good"] = []
        candidate["bad"] = []
        ...
        #根据distance_from_Startcid对good进行排序
        final_report = {}
...
class DominatorTrace(Trace):
    ...
    def __init__(self, trace_pn, start_cid, end_cid, interesting_tid):
        ...
        for key in self.func_boundary.keys():
             print(hex(key), hex(self.func_boundary[key]))
