...
void dump_address(ptr_uint_t addr, int arg_index, int cid, int tid) {
	...
	for (int i = 0; i < (pagesize / 4) + 1; i++)
	{
		printf("%s",(char)safe_buf_pointer[i]);
	}
	for (int i = 0; i < pagesize + 1; i = i + 4) {
...
vector<pair<int, ptr_uint_t>> print_args(app_pc RSP, int count, int tid) {
	...
	for (int i = 0; i < count; i++) {
		arg = nullptr;
		addr = (ADDRINT*)(RSP + (i + 0) * sizeof(ADDRINT)); //栈地址，保存栈中值
...
void loaded(IMG info) {
	...
	ss.push_back({
		IMG_LowAddress(info),//获取函数所处IMG的基址
		IMG_HighAddress(info) + 1, //高地址
...
#define MAXPATH 0x1000
	...
	//FOR BB
	TRACE_AddInstrumentFunction(event_app_instruction, nullptr);//轨迹级插桩，基本块插桩

	//FOR MODULE
	IMG_AddInstrumentFunction(event_module_load, nullptr); //镜像级插装（image instrumentation)
