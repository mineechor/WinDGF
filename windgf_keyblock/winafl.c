                      ...
                      bool for_trace, bool translating, void *user_data)
{
    ...
    for(i=0;i<bbnum-1;i++)
    {
        if(off>=bbwei[i].start&&off<bbwei[i].end)
        {
            ...
            instrlist_meta_preinsert(bb, inst,
    LOCK(INSTR_CREATE_add(drcontext,
                               OPND_CREATE_ABSMEM(&count, OPSZ_4),OPND_CREATE_INT8(1))));
            break;
        }
...
pre_fuzz_handler(void *wrapcxt, INOUT void **user_data)
{
    ...
    count1 = 0;
    // num = 0;
    // fprintf(f,"begindis %lld\n",winafl_data.dis[0]);
    // fprintf(f,"beginnum %lld\n",winafl_data.dis[1]);
    if(options.coverage_kind == COVERAGE_EDGE || options.thread_coverage) {
        void **thread_data = (void **)drmgr_get_tls_field(drcontext, winafl_tls_field);
        thread_data[0] = 0;
        thread_data[1] = winafl_data.afl_area;
...
post_fuzz_handler(void *wrapcxt, void *user_data)
{
    ...
    // fprintf(f,"%lld\n",winafl_data.dis[0]);
    // fprintf(f,"err %d\n",GetLastError());
    // fprintf(f,"enddis %lld\n",winafl_data.dis[0]);
    // fprintf(f,"endnum %lld\n",winafl_data.dis[1]);
    // fclose(f);
    if(!options.debug_mode) {
		WriteCommandToPipe('K');
    } else {
...
dr_client_main(client_id_t id, int argc, const char *argv[])
{
    ...
    // f = fopen("D:\\final\\2022\\winafl\\winafldirctedgo3key\\build32\\bin\\Release\\log.txt","a");
    // fprintf(f,"yes\n");
    // char* dir;
    // fprintf(f,"%s\n",options.weightdir);
    // weigh = options.weightdir;
    bbfile = fopen("D:\\final\\2022\\winafl\\winafldirctedgo3key\\build32\\bin\\Release\\bblock.txt","r");
    // fprintf(f,"%d\n",feof(bbfile));
    // char line[1024];
    // size_t len = 0;
    bbnum = 0;
    ...
    while(!feof(bbfile))
    {
        fscanf(bbfile,"%u %u",&bbwei[bbnum].start,&bbwei[bbnum].end);
