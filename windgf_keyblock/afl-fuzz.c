...
static double count_distance(u8* mem)
{
  double dis = 0;
  u32 total = 0;
  u32 i =0;
  ...
  for (i=0;i<9;i++)
  {
    ...
    {
      total = total+trace_bits[MAP_SIZE+i]*(u32)pow(10,i);
    }
  }
  num = trace_bits[MAP_SIZE+9];
  // fprintf(fd,"num%d\n",num);
  // fprintf(fd,"dis%d\n",dis);
  if(num)
  dis = (double)total/(double)num;
  else
  dis = -1.0;
  return dis;
}
...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  if(if_directed)
  {
    // cur_distance = count_distance(trace_bits);
    if(*total_num>0)
      cur_distance = (double)(*total_dis)/(double)(*total_num);
    else
      cur_distance = -1.0;
  }
#else
  ...
  if(if_directed)
  {
    // cur_distance = count_distance(trace_bits);
    if(total[1]>0)
      cur_distance = (double)(total[0])/(double)(total[1]);
    else
      cur_distance = -1.0;
    // fprintf(fd,"cur%d\n",cur_distance);
  }
...
static void update_bitmap_score(struct queue_entry* q) {
  ...
  // if(if_directed)
  // {
  // count = count_distance(trace_bits);
  // SAYF("%d\n",count);
  // }
  ...
  for (i = 0; i < MAP_SIZE; i++)
    ...
    if (trace_bits[i]) {
        // printf("%d",trace_bits[i]);
       if (top_rated[i]) {
        //  if((if_directed&&(q->j_distance>top_rated[i]->j_distance))||(if_directed&&!q->j_distance&&top_rated[i]->j_distance))
        //    continue;
        //  else
        //   {
...
static void setup_shm(void) {
  ...
  while(attempts < 5) {
    ...
    shm_handle = CreateFileMapping(
                   ...
                   MAP_SIZE+12,                // maximum object size (low-order DWORD)
  ...
  if (!total) PFATAL("disshmat() failed");
  // if (!total_num) PFATAL("numshmat() failed");
...
//   for(i=0;i<10;i++)
//   {
//   fprintf(fd,"%d",trace_bits[MAP_SIZE+i]);
//   }
//   fprintf(fd,"\n");
// }
#ifdef __x86_64__
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (q->exec_cksum != cksum) {
      ...
      if (q->exec_cksum) {
        ...
        u32 i;
        // if(q->j_distance!=count_distance(trace_bits)||!q->j_distance)
        //   stage_max = CAL_CYCLES_MID;
      ...
      // else if(q->j_distance!=count_distance(trace_bits))
      // {
      //   stage_max    = CAL_CYCLES_LONG;
      // }
      else {
  ...
  // if(if_directed)
  // {
  // q->j_distance = count_distance(trace_bits);
  // total_count_distance += q->j_distance;
  // fprintf(fd,"caliend%d\n",q->j_distance);
  // }
  q->handicap    = handicap;
  q->cal_failed  = 0;
...
static u8 trim_case(char** argv, struct queue_entry* q, u8* in_buf) {
  ...
  static u8 tmp[64];
  static u8 clean_trace[MAP_SIZE+10];
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  if (q->j_distance > 0) {
    ...
    if (max_distance != min_distance)
      normalized_d = (q->j_distance - min_distance) / (max_distance - min_distance);
    fprintf(fd,"normal %lf\n",normalized_d);
    if (normalized_d >= 0) {
...
static u8 fuzz_one(char** argv) {
  ...
  if (pending_favored) {
    ...
    //并且生成一个100以内的随机数，如果小于SKIP_TO_NEW_PROB(99%)直接return 1
    ...
    if (queue_cycle > 1 && !queue_cur->was_fuzzed) {
      ...
      //否则打一个100以内的随机数，如果小于这个直接return 1（95%）
...
//  fprintf(fd,"calibrate end\n");
  /************
   * TRIMMING *
   ************/
//  fprintf(fd,"trim_case begin\n");
...
int main(int argc, char** argv) {
  ...
  char** use_argv;
  fd = fopen("D:\\final\\2022\\winafl\\winafldirctedgo3key\\build32\\bin\\Release\\log2.txt","a");
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:I:T:rdYnCB:S:M:x:QD:z:c:b:l:pP")) > 0)

    switch (opt) {
      //此选项表示待测目标输入文件所在的目录，接受一个目录参数
      case 'r':
        if(if_directed) FATAL("Multiple -r options not supported");
        if_directed = TRUE;
        break;
      case 'i':
        if (in_dir) FATAL("Multiple -i options not supported"); //首先检查indir是否已被设置，防止多次设置-i选项。
        in_dir = optarg;

        if (!strcmp(in_dir, "-")) in_place_resume = 1; //标志位
      ...
      case 'M': { /* master sync ID */
        ...
        //检查sync_id是否已被设置，防止多次设置-M/-S选项
          ...
          force_deterministic = 1; //标志位
          fuzzer_id = sync_id;

        }
        break;

      case 'S': //-M -S 选项指定 主fuzzer 和 从fuzzer ，用于多线程的fuzz
      ...
      case 'm': { 
          ...
          u8 suffix = 'M';  //指定内存限制，由于ASAN的使用可能导致内存消耗过大，一般用作解除限制
      ...
      //用于启用fuzz测试实例的快速模式（快速模式下将跳转确定性检查步骤，这将导致误报率显著上升）
      case 'd':
        ...
        use_splicing = 1;
        break;
      //此选项是一个隐藏的非官方选项，如果在测试过程中发现了一个有趣的测试用例，想要直接基于此且不想重新进行早期的样本变异，可以使用此选项直接指定一个bitmap文件
      case 'B':
        ...
        break;
      /*基于覆盖率的fuzz中通常会生成一个崩溃分组的小数据集，可以手动或使用非常简单的GDB或Valgrind脚本进行快速分
      类。这使得每个崩溃都可以追溯到队列中的非崩溃测试父用例，从而更容易诊断故障。但是如果没有大量调试和代码
      分析工作，一些模糊测试崩溃可能很难快速评估其可利用性。为了协助完成此任务，afl-fuzz支持使用-C标志启用的
      非常独特的“崩溃探索”模式。在这种模式下，模糊器将一个或多个崩溃测试用例作为输入，并使用其反馈驱动的模糊测
      试策略非常快速地枚举程序中可以到达的所有代码路径，同时保持程序处于崩溃状态。此时，fuzz器运行过程中生成
      的不会导致崩溃的样本变异被拒绝，任何不影响执行路径的变异也会被拒绝。
      */
      case 'C':
      ...
      default:
        usage(argv[0]); //调用usage函数打印afl-fuzz的用法
  ...
  read_testcases(); //读取测试用例到队列，在启动时调用
  ...
  //检查目标文件的有效性，是否是可执行文件，是否是Mach-0还是ELF还是一个生成的shell文件
  ...
  //仅对初始输入执行此操作，并且仅执行一次
  //  fprintf(fd,"perform begin\n");
  perform_dry_run(use_argv); //首先跑一遍所有的测试用例，记录信息到样本队列
  // fprintf(fd,"perform end\n");
  //精简队列
  cull_queue();
  //向用户输出一些状态信息
  show_init_stats();
  ...
  while (1) {
    ...
    if (!queue_cur) { //说明此时整个队列已经扫描了一遍了
      ...
      while (seek_to) {  
        current_entry++; 
        seek_to--; 
        queue_cur = queue_cur->next; //顺着队列持续后移
        //感觉就是把queue_cur置位到seek_to的位置
      }
      // fprintf(fd,"seekid %d\n",current_entry);
      //展示状态
      show_stats();
      ...
      //如果经历了一整个完整的扫描周期后都没有新的路径发现，尝试调整策略
      if (queued_paths == prev_queued) {

        if (use_splicing) cycles_wo_finds++; else use_splicing = 1; //代表我们要通过splicing进行队列重组
    ...
    //如果skipped_fuzz为0，并且stop_soon为0，并且设置了sync_id
    if (!stop_soon && sync_id && !skipped_fuzz) {
      //若sync_interval_cnt没有到一个周期，调用sync_fuzzers同步其他fuzzer
      if (!(sync_interval_cnt++ % SYNC_INTERVAL))
        sync_fuzzers(use_argv);

    }
    if (stop_soon) break;
    //如果没有设置stop_soon，准备下一个样本
    queue_cur = queue_cur->next;
    current_entry++;
    // fprintf(fd,"endid %d",current_entry);
